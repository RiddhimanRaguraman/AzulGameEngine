...
namespace Azul
{
	void ConvertSkin(const char* const pFileName, const char* const pTargetName)
	{
		...
		std::vector<std::pair<int, int>> meshSkinPairs;
		for (int mi = 0; mi < (int)gltfModel.meshes.size(); mi++)
		{
			for (size_t ni = 0; ni < gltfModel.nodes.size(); ni++)
			{
				const auto& node = gltfModel.nodes[ni];
				if (node.mesh == mi && node.skin >= 0)
				{
					meshSkinPairs.push_back({ mi, node.skin });
					break;
				}
			}
		}
		assert(!meshSkinPairs.empty());
	...
	/*	Skin0.jointTable = gltfModel.skins[0].joints;
		...
		for (size_t outIndex = 0; outIndex < meshSkinPairs.size(); outIndex++)
		{
			const int meshIndex = meshSkinPairs[outIndex].first;
			const int skinIndex = meshSkinPairs[outIndex].second;
			assert(meshIndex >= 0);
			...
			int primitiveIndex = -1;
			{
				...
				for (size_t i = 0; i < gltfModel.meshes[(size_t)meshIndex].primitives.size(); i++)
				{
					...
					if (attrs.find("JOINTS_0") != attrs.end() && attrs.find("WEIGHTS_0") != attrs.end() && attrs.find("POSITION") != attrs.end())
					{
			...
			const size_t inverseMatrixAccessorIndex = (size_t)gltfModel.skins[(size_t)skinIndex].inverseBindMatrices;
			const auto& prim = gltfModel.meshes[(size_t)meshIndex].primitives[(size_t)primitiveIndex];
			const size_t weightAccessorIndex = (size_t)prim.attributes.find("WEIGHTS_0")->second;
			const size_t jointAccessorIndex = (size_t)prim.attributes.find("JOINTS_0")->second;
			const size_t indexAccessorIndex = (size_t)prim.indices;
			const size_t posAccessorIndex = (size_t)prim.attributes.find("POSITION")->second;
			const size_t normsAccessorIndex = (size_t)prim.attributes.find("NORMAL")->second;
			const size_t texCoordAccessorIndex = (size_t)prim.attributes.find("TEXCOORD_0")->second;
			...
			Skin0.skinName = gltfModel.meshes[(size_t)meshIndex].name;
			...
			GLTF::SetCustomVBO(runModel.vbo_vert,
				(char*)&Skin0.pos[0],
				(uint32_t)Skin0.pos.size() * sizeof(Vec3f),
				(uint32_t)Skin0.pos.size(),
			...
			GLTF::SetCustomVBO(runModel.vbo_norm,
				(char*)&Skin0.norms[0],
				(uint32_t)Skin0.norms.size() * sizeof(Vec3f),
				(uint32_t)Skin0.norms.size(),
				vboData::VBO_COMPONENT::FLOAT,
				vboData::VBO_TYPE::VEC3,
				vboData::VBO_TARGET::ARRAY_BUFFER
			);
			...
			GLTF::SetCustomVBO(runModel.vbo_uv,
				...
				(uint32_t)Skin0.text_coord.size() * sizeof(Vec2f),
				(uint32_t)Skin0.text_coord.size(),
				vboData::VBO_COMPONENT::FLOAT,
			...
			GLTF::SetCustomVBO_index(runModel.vbo_index,
				(char*)&Skin0.index[0],
				(uint32_t)Skin0.index.size() * sizeof(Vec3ui),
				(uint32_t)Skin0.index.size() * 3,
			...
			GLTF::SetCustomVBO(runModel.vbo_weights,
				...
				(uint32_t)Skin0.weights.size() * sizeof(Vec4f),
				(uint32_t)Skin0.weights.size(),
				vboData::VBO_COMPONENT::FLOAT,
			...
			GLTF::SetCustomVBO(runModel.vbo_joints,
				(char*)&Skin0.joints[0],
				(uint32_t)Skin0.joints.size() * sizeof(Vec4ui),
				(uint32_t)Skin0.joints.size(),
				vboData::VBO_COMPONENT::UNSIGNED_INT,
				vboData::VBO_TYPE::VEC4,
				vboData::VBO_TARGET::ARRAY_BUFFER
			);
			...
			GLTF::SetCustomVBO(runModel.vbo_invBind,
				...
				(uint32_t)Skin0.invBone.size() * sizeof(Mat4),
				(uint32_t)Skin0.invBone.size(),
				vboData::VBO_COMPONENT::FLOAT,
			...
			runModel.mode = meshDataConverter::GetMode(gltfModel.meshes[(size_t)meshIndex].primitives[(size_t)primitiveIndex].mode);
			...
			std::string k = "";
			if (meshSkinPairs.size() > 1)
			{
				k = std::to_string(outIndex);
			}
