...
namespace Azul
{
	...
	CompareStrategyBase *ManBase::posDefaultCompareStrategy = nullptr;
	...
	ManBase::ManBase(ListBase *_poActive, ListBase *_poReserve, int InitialNumReserved, int DeltaGrow)
	{
		...
		if(ManBase::posDefaultCompareStrategy == nullptr)
		{
			ManBase::posDefaultCompareStrategy = new CompareStrategyBase();
			assert(ManBase::posDefaultCompareStrategy);
		}

		this->pCompareStrategy = ManBase::posDefaultCompareStrategy;
		assert(this->pCompareStrategy);
	}

	ManBase::~ManBase()
	{
		delete ManBase::posDefaultCompareStrategy;
		ManBase::posDefaultCompareStrategy = nullptr;
	...
	DLink *ManBase::baseFind(DLink *pRefNode)
	{
		assert(pRefNode);
		DLink *pNode = nullptr;
		...
		for (pIt->First(); !pIt->IsDone(); pIt->Next())
		{
			...
			if(this->pCompareStrategy->Compare(pRefNode,pNode))
			{
				// found it
				break;
			}
