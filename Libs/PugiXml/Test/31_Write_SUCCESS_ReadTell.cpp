...
// used for teardown
static File::Handle fh_file31;
static File::Handle fh_file31b;
...
TEST_WITH_TEARDOWN(Write_SUCCESS_ReadTell, TestConfig::ALL)
{
#if Write_SUCCESS_ReadTell
	...
	ferror = File::Open(fh, "file31.bin", File::Mode::WRITE);
	fh_file31 = fh;
	...
	ferror = File::Open(fh, "file31.bin", File::Mode::READ);
	fh_file31b = fh;
	CHECK(ferror == File::Error::SUCCESS);
	CHECK(fh != nullptr);
...
TEST_TEARDOWN(Write_SUCCESS_ReadTell)
{
#if Write_SUCCESS_ReadTell

	DWORD flags;
	BOOL status = GetHandleInformation(fh_file31, &flags);
	if (status)
	{
		File::Close(fh_file31);
	}

	status = GetHandleInformation(fh_file31b, &flags);
	if (status)
	{
		File::Close(fh_file31b);
	}
