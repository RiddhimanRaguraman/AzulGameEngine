...
TEST_WITH_TEARDOWN(Write_SUCCESS_Read2, TestConfig::ALL)
{
#if Write_SUCCESS_Read2
	...
	ferror = File::Open(fh, "file28.bin", File::Mode::READ_WRITE);
	fh_file28 = fh;
...
TEST_TEARDOWN(Write_SUCCESS_Read2)
{
#if Write_SUCCESS_Read2

	DWORD flags;
	const BOOL status = GetHandleInformation(fh_file28, &flags);
	if (status)
	{
		File::Close(fh_file28);
	}
