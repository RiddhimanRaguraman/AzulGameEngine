...
void main(uint3 dtID : SV_DispatchThreadID)
{
    ...
    if (dtID.x < csMixerB.numNodes)
    {
        uMixerOutBx[boneIndex] = tKeyBa[boneIndex];
  
        float4 trans = lerpMe(tKeyBa[boneIndex].t, tKeyBb[boneIndex].t, csMixerB.ts);
        float4 quat = slerp(tKeyBa[boneIndex].q, tKeyBb[boneIndex].q, csMixerB.ts);
        float4 scale = lerpMe(tKeyBa[boneIndex].s, tKeyBb[boneIndex].s, csMixerB.ts);

        uMixerOutBx[boneIndex].t = trans;
        uMixerOutBx[boneIndex].q = quat;
        uMixerOutBx[boneIndex].s = scale;
    }
