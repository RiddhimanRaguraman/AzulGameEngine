...
namespace Azul
{
	...
	BufferTextureSRV_ps::BufferTextureSRV_ps()
	...
	void BufferTextureSRV_ps::Initialize(textureData::TEXTURE_NCOMPONENT Component,
	...
	void BufferTextureSRV_ps::privCreate(BufferTexture2D &rTex2D)
	{
		...
		hr = StateDirectXMan::GetDevice()->CreateShaderResourceView(poTex,
																	&SRVDesc,
																	&poSRV);
		assert(SUCCEEDED(hr));
	}

	void BufferTextureSRV_ps::SetActive(ShaderResourceBufferSlot slot)
	{
		assert(bCreate == true);
		StateDirectXMan::GetContext()->PSSetShaderResources((uint32_t)slot,
															1,
															&this->poSRV);
	}
