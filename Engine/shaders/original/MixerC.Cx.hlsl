...
#define UAV_uMixerOutBx
#define UAV_uMixerOutCx
#define CBV_csMixerC

#include "ShaderMappings.hlsli"

// same as MixerA... but inputs Key are UAV not SRV
...
void main(uint3 dtID : SV_DispatchThreadID)
{
    ...
    if (dtID.x < csMixerC.numNodes)
    {
        uMixerOutCx[boneIndex] = uMixerOutAx[boneIndex];
  
        float4 trans = lerpMe(uMixerOutAx[boneIndex].t, uMixerOutBx[boneIndex].t, csMixerC.ts);
        float4 quat = slerp(uMixerOutAx[boneIndex].q, uMixerOutBx[boneIndex].q, csMixerC.ts);
        float4 scale = lerpMe(uMixerOutAx[boneIndex].s, uMixerOutBx[boneIndex].s, csMixerC.ts);

        uMixerOutCx[boneIndex].t = trans;
        uMixerOutCx[boneIndex].q = quat;
        uMixerOutCx[boneIndex].s = scale;
    }
