...
namespace Azul
{
		...
		poWorldComputeC{nullptr}
	{
		...
		// --- Mixer C ----
		HierarchyTable::Name h0 = pClipA->GetHierarchyName();
		HierarchyTable::Name h1 = pClipB->GetHierarchyName();
		assert(h0 == h1);

		AZUL_UNUSED_VAR(h1);

		HierarchyTable *pHierarchyTable0 = HierarchyTableMan::Find(h0);
	...
	void ComputeBlend_TwoAnim::AnimateMixerC()
	{
		this->poMixerC->pKeyCa = &this->poMixerA->mMixerResult;
		this->poMixerC->pKeyCb = &this->poMixerB->mMixerResult;
		this->poMixerC->poMixerConstant->ts = this->mBlendTs;

	}

	void ComputeBlend_TwoAnim::SetBlendTs(float ts)
	{
		this->mBlendTs = ts;
	}

	float ComputeBlend_TwoAnim::GetBlendTs() const
	{
		return this->mBlendTs;
	}
