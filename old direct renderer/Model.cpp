...
Model::Model(ID3D11Device* dev, Model::PreMadeModels pm)
{
	...
	case UnitBoxRepeatedTexture:
		...
		break;
	case UnitBoxUniqueTexture:
		ModelTools::CreateUnitBoxUniqueTexture(pStdVerts, numVerts, pTriList, numTris);
		break;
	case UnitPyramid:
		ModelTools::CreateUnitPyramid(pStdVerts, numVerts, pTriList, numTris);
		break;
	case UnitPyramidRepTexture:
		ModelTools::CreateUnitPyramidRepTexture(pStdVerts, numVerts, pTriList, numTris);
		break;
	case UnitSphere:
		ModelTools::CreateUnitSphere(24, 24, pStdVerts, numVerts, pTriList, numTris);
		break;
	default:
...
void Model::privLoadDataFromFile(const char * const _modelName, StandardVertex*& pVerts, int& nverts, TriangleByIndex*& ptlist, int& ntri)
{
	...
	for (int i = 0; i < nverts; i++)
	{
		tmp = &(pTmpVerts[i]);
		v = &(pVerts[i]);
		v->set(tmp->x, tmp->y, tmp->z, tmp->u, 1.0f - tmp->v, tmp->nx, tmp->ny, tmp->nz, tmp->r, tmp->g, tmp->b, (int) tmp->txt);
	}
