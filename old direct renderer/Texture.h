...
public:
	...
	Texture(ID3D11Device* d3dDev, LPCWSTR filepath, D3D11_FILTER filter = D3D11_FILTER_MIN_MAG_MIP_LINEAR, UINT isotropic_level = 4, bool ComputeMip = false, size_t miplevel = 0, DirectX::TEX_FILTER_FLAGS filterflags = DirectX::TEX_FILTER_MIRROR);

	ID3D11ShaderResourceView* GetTextureResource() const { return mpTextureRV; }
	ID3D11SamplerState* GetSampler() const { return mpSampler; }
